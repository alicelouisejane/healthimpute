...
#'
#' The function supports individuals aged 0 to 20 years using the CDC infant (0–24 months) and child/adolescent (>24–240 months) growth standards.
#'
 ...
 impute_anthro <- function(data, variable,patient_id_col,age_col,sex_col) {
   ...
   data_new <- dplyr::bind_rows(data_adults, data_inf)%>%
     dplyr::mutate(
       ...
       percentile = pmin(stats::pnorm(z) * 100, 99.999)) %>% # cap as cant be 100th
     dplyr::group_by(!!id_sym) %>%
     dplyr::arrange(!!age_sym, .by_group = TRUE) %>%
     dplyr::mutate(
       last_percentile = lag(percentile),
       next_percentile = lead(percentile)) %>%
     dplyr::mutate(impute_percentile = dplyr::case_when(
         is.na(percentile) & !is.na(last_percentile) & !is.na(next_percentile) ~ (last_percentile + next_percentile) / 2,
         is.na(percentile) & !is.na(last_percentile) ~ last_percentile,
         is.na(percentile) & !is.na(next_percentile) ~ next_percentile,
         TRUE ~ NA_real_
       )
     ) %>%
     dplyr::mutate(impute_percentile = ifelse(is.na(impute_percentile) & !is.na(percentile),percentile,impute_percentile)) %>%
     #carry forward or back where there is >1 missing consecutive percentile
     tidyr::fill(impute_percentile,.direction="downup") %>%
     dplyr::ungroup()
